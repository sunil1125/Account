...
!(function ($that) {
    ...
    btTypeAhead.Constructor.prototype.getDataItems = function (item) {
        var dataKeys = this.getDataKeys(), dItem = [];
    ...
    btTypeAhead.Constructor.prototype.process = function (results) {
        ...
        if (that.focused) {
            ...
            if (items && items.length === 1) {
                var firstItem = items[0], selectedText = '';
                if (!that.isResultItemIsString) {
                ...
                if ((selectedText && selectedText.toLowerCase().indexOf(that.query.toLowerCase()) === -1) && !(that.showEmptyMessage && selectedText.toLowerCase().indexOf(that.msgNoRecordFound.toLowerCase())) && !that.serverSideFilter) {
                    return;
                } else {
                    return that.show();
                }
    ...
    btTypeAhead.Constructor.prototype.sorter = function (items) {
        var beginswith = [], caseSensitive = [], caseInsensitive = [], item, sortby;

        while (item = items.shift()) {
            if (this.isResultItemIsString)
                sortby = item;
else if (typeof item[this.getFilterKey()] === 'function') {
                sortby = item[this.getFilterKey()]();
            } else {
                sortby = item[this.getFilterKey()];
            }

            if (!sortby.toLowerCase().indexOf(this.query.toLowerCase()))
                beginswith.push(item);
else if (~sortby.indexOf(this.query))
    ...
    btTypeAhead.Constructor.prototype.render = function (items) {
        ...
        items = $that(items).map(function (index, item) {
            var liItem = $that(that.options.item).attr('data-value', JSON.stringify(item)), dataItem = item, iconSettings = that.$element.data('iconsettings');
            ...
            if (typeof iconSettings === 'object') {
                var spanText = $('<span></span>').html(that.highlighter(item)), iconClass = null;
...
!(function (knockout) {
        ...
        update: function (element, valueAccessor, allBindingsAccessor) {
            ...
            if (widget) {
                ...
                if (widget.date instanceof Date) {
                    ...
                    var widgetDate = CommonUtils.formatDate(widget.date, widget.format);
                ...
                if (widget.blockPreviousDays) {
                    if (options.startDate instanceof Date) {
                        if (widget.startDate instanceof Date) {
                            ...
                            var CommonUtils = new Utils.Common();
                            var widgetStartDate = CommonUtils.parseDate(widget.startDate, widget.format);
    ...
    */
    knockout.bindingHandlers.showContentProgress_old = {
        update: function (element, valueAccessor, allBindingsAccessor) {
            var value = ko.utils.unwrapObservable(valueAccessor()), defaults = {
                ...
                loaderText: ''
            }, contentProgressOptions = typeof allBindingsAccessor().contentProgressOptions === 'object' && allBindingsAccessor().contentProgressOptions, options = jQuery.extend({}, defaults, contentProgressOptions), hasChildId = (options.childId && options.childId.length) ? true : false, container = hasChildId ? $(element).children('#' + options.childId) : $(element);
            if (!container.length) {
            ...
            if (value) {
                ...
                if (!ajaxOverlay.length) {
                    var overlay = $('<div></div>').css({
                        'background-color': options.bgColor,
                        'opacity': options.opacity,
                        'width': container.css('width').search("%") == -1 && container.width() ? container.width().toString() : '100%',
                        'height': contentProgressOptions.height ? contentProgressOptions.height : (container.css('height').search("%") == -1 && container.height() ? container.height().toString() : '100%'),
                        'position': 'absolute',
                        'bottom': '0px',
                        'left': '0px',
                        'right': '0px',
    ...
    */
    knockout.bindingHandlers.accessRule = {
        init: function (element, valueAccessor, allBindingsAccessor) {
            var value = ko.utils.unwrapObservable(valueAccessor());
            var accessRuleOptions = (typeof allBindingsAccessor().accessRuleOption === 'object' && allBindingsAccessor().accessRuleOption);
            var options = jQuery.extend({}, accessRuleOptions);
            if (!options) {
            ...
            var accessType = options.AccessType || 'disable';
            ...
            if (value instanceof Array && value && value.length) {
                var rule = ko.utils.arrayFirst(value, function (item) {
    ...
    knockout.bindingHandlers.daterangepicker = {
        init: function (element, valueAccessor, allBindingsAccessor) {
            ...
            /* when a user changes the date, update the view model */
            ko.utils.registerEventHandler(element, "changeDateRange", function (event) {
            ...
            if (value && element.disabled) {
                ...
                $(element).each(function () {
                    ...
                    $(currentElement).find('*').filter(function (i, el) {
                        if ($(this).data('bind')) {
                            if (($(this).data('bind')).indexOf('enable:') === -1) {
    ...
    */
    ...
    knockout.bindingHandlers.slideDialog = {
        init: function (element, valueAccessor) {
            ...
            if (ko.utils.unwrapObservable(value)) {
                ...
                var body = $('body');
                var blockout = $('<div class="modalBlockout"></div>').css({ 'z-index': '1051', 'opacity': '0.1', 'background': 'white', 'background-position': 'initial initial', 'background-repeat': 'initial initial' }).appendTo(body);
                var host = $('<div class="modalHost"></div>').css({ 'z-index': '1052', 'opacity': '1.0' }).appendTo(body).appendTo(host);
                ;
    ...
    knockout.bindingHandlers.radioCheck = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            ...
            var radioCheckOptions = allBindingsAccessor().radioCheckOptions, bindingcontext = radioCheckOptions.bindingcontext, valuekey = radioCheckOptions.valuekey;
    ...
    */
    ...
    knockout.bindingHandlers.jQTimepicker = {
        init: function (element, valueAccessor, allBindingsAccessor) {
            var options = allBindingsAccessor().jQTimepickerOptions || {};
            ...
            ko.utils.registerEventHandler(element, "changeTime.jQTimepicker", function (event) {
                var value = valueAccessor();
                if (ko.isObservable(value)) {
                    value(event.time);
                }
        ...
        update: function (element, valueAccessor, allBindingsAccessor) {
            ...
            var value = ko.utils.unwrapObservable(valueAccessor()), options = allBindingsAccessor().jQTimepickerOptions || {}, widgetData = $(element).data(), isTimeUpdatedInUi = ($(element).val() && $(element).val().length) ? true : false;
            ...
            if ($(element).val() !== value) {
                ...
                if (!(options).equals(widgetData.timepickerOptions)) {
    ...
    ko.bindingHandlers.numeric = {
        init: function (element, valueAccessor) {
            $(element).on('keydown', function (event) {
                if (event.keyCode == 46 || event.keyCode == 8 || event.keyCode == 9 || event.keyCode == 27 || event.keyCode == 13 || (event.ctrlKey === true) || (event.keyCode === 190) || (event.keyCode >= 35 && event.keyCode <= 39)) {
    ...
    if (thatkg) {
        ...
        */
        ...
        kgsortService.getSortItemType = function (sortField, data) {
            ...
            var sortFieldType, item, sortDir = DESC;
            item = ko.utils.arrayFirst(unwrappedData, function (i) {
...
!(function (a) {
    ...
    a.prototype.removeAll = function (items) {
        var self = this;
        ...
        return self.remove(function (element) {
            var match = -1;
    ...
    a.prototype.remove = function (item) {
        var self = this;
    ...
    a.prototype.arrayIndexOf = function (predicate) {
        var array = this;
    ...
    a.prototype.arrayIndexOfValue = function (searchForValue, predicate) {
        var array = this;
        if (!predicate || typeof (predicate) != 'function') {
            return -1;
        }
        ...
        for (var i = 0; i < array.length; i++) {
            if (predicate(array[i], searchForValue))
                return i;
        }
...
!(function (s) {
    ...
    */
    s.prototype.capitalize = function (all) {
        var self = this;
        if (all) {
...
(function (Utils) {
    var Common = (function () {
        ...
        Common.prototype.parseDateFormat = function (format) {
            if (!(format)) {
                format = 'mm/dd/yyyy';
            }
        ...
        Common.prototype.parseDate = function (date, format) {
            if (!(format) || !(format.separator)) {
                format = this.parseDateFormat(format);
            }
            var parts = date.split(format.separator), dateNew = new Date(), val;
            dateNew.setHours(0);
        ...
        Common.prototype.convertToDate = function (date) {
        ...
        Common.prototype.convertToDateString = function (date, format) {
        ...
        */
        Common.prototype.getEnumValueById = function (obj, id) {
            for (var i in obj) {
        ...
        * @returns {string} value of the given input from the enum object.
        */
        Common.prototype.getEnumValueByKey = function (obj, key) {
            for (var i in obj) {
        ...
        * @returns {string} value of the given input from the enum object.
        */
        Common.prototype.getEnumIdByValue = function (obj, key) {
            for (var i in obj) {
        ...
        */
        Common.prototype.roundOff = function (value, decimals) {
        ...
        Common.prototype.ConvertTime12Hour = function (dateTime, separator) {
        ...
        Common.prototype.deepCopy = function (source) {
        ...
        * @returns lbs value
        */
        Common.prototype.ConvertToLbs = function (value, valueType) {
        ...
        */
        Common.prototype.isBrowserIsIE = function () {
        ...
        */
        Common.prototype.isBrowserIsFirefox = function () {
        ...
        */
        Common.prototype.isBrowserIsChrome = function () {
        ...
        */
        Common.prototype.isBrowserIsSafari = function () {
        ...
        */
        Common.prototype.UserAgent = function () {
        ...
        */
        Common.prototype.isValidDate = function (value, userFormat) {
            var userFormat = userFormat || 'mm/dd/yyyy', delimiter = /[^mdy]/.exec(userFormat)[0], theFormat = userFormat.split(delimiter), theDate = value.split(delimiter), isDate = function (date, format) {
                var m, d, y;
        ...
        Common.prototype.isWeekend = function (date) {
            var day;
        ...
        Common.prototype.isHoliday = function (date) {
            if (date instanceof Date) {
        ...
        Common.prototype.nextWorkingDay = function (value) {
            var date = new Date(), isValidDate = false;
        ...
        */
        function (a, b) {
            var alphaRE = /[^a-zA-Z0-9]/g, numberRE = /[^0-9]/g, aA = a ? a.toLowerCase().replace(alphaRE, "") : a, bA = b ? b.toLowerCase().replace(alphaRE, "") : b;
            if (aA === bA) {
        ...
        */
        Common.prototype.isNullOrEmptyOrWhiteSpaces = function (value) {
            if (value && value.trim()) {
                return true;
            } else {
                return false;
        ...
        */
        Common.prototype.USAPhoneFormat = function (input) {
    ...
    */
    function applyAccessType(element, accessType) {
    ...
    Utils.applyAccessType = applyAccessType;
    ...
    */
    function getDirtyItems(myModel) {
        var dirtyItems = [];
        if (myModel) {
    ...
    Utils.getDirtyItems = getDirtyItems;
    ...
    */
    function getChangesFromModel(viewmodel) {
        var changedItems = [];
        if (viewmodel) {
    ...
    Utils.getChangesFromModel = getChangesFromModel;
    ...
    */
    function removeNumberWithbrackets(input) {
        if (input) {
    ...
    Utils.removeNumberWithbrackets = removeNumberWithbrackets;
    ...
    * @returns empty.
    */
    function kgSetClientsidePagination(data, gridOption, grid, currentData) {
        try  {
            var pgngOption = gridOption.gridOptions.pagingOptions, pagedData = data.slice((pgngOption.currentPage() - 1) * pgngOption.pageSize(), pgngOption.currentPage() * pgngOption.pageSize()), fltrOptions = gridOption.gridOptions.filterOptions;
    ...
    Utils.kgSetClientsidePagination = kgSetClientsidePagination;
    ...
    */
    function addObjectTagToControl(targetID, sourceURL, mimeType, alternativeURL) {
    ...
    Utils.addObjectTagToControl = addObjectTagToControl;
