...
interface KnockoutValidationStatic {
	...
	addAnonymousRule<T>(observable: KnockoutObservableBase<T>, ruleObj: KnockoutValidationAnonymousRuleDefinition): void;

    insertValidationMessage(element: Element): Element;
	parseInputValidationAttributes<T>(element: Element, valueAccessor: () => KnockoutObservableBase<T>): void;
	...
	validateObservable<T>(observable: KnockoutObservableBase<T>): boolean;
}

interface KnockoutStatic {
    validation: KnockoutValidationStatic;
	validatedObservable<T>(initialValue: any): KnockoutObservableBase<T>;
