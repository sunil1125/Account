...
/***********************************************
** <changeHistory>
** <id>US23323</id> <by>Janakiram</by> <date>14-07-2016</date> <description> Email field should allow multiple Emails and validation</description>
** <id>DE23551</id> <by>Janakiram</by> <date>03-08-2016</date> <description> Email field should allow multiple Emails and validation</description>
** </changeHistory>
***********************************************/
(function () {
	...
	ko.validation = (function () {
		...
		return {
			...
			addAnonymousRule: function (observable, ruleObj) {
				...
				if (observable.anonymousRule && observable.anonymousRule.length) {
					observable.anonymousRule.push(ruleName);
				} else {
					observable.anonymousRule = [ruleName];
				}
	...
	ko.validation.rules['multiemail'] = {
		...
		message: 'Please enter valid email addresses (separate multiple email addresses using a comma or semicolon).'
	};
	...
	ko.validation.rules['ediValidaton'] = {
		validator: function (val, params) {
			if (params && val)
				return (/^[^>*~]+$/g).test(val);
			return true;
		},
		message: '<span style="font-weight:bold;font-size:14px;">~, *</span> and <span style="font-weight:bold;font-size:14px;">></span> are NOT allowed.',
		html: true
	};
	...
	ko.extenders['validatable'] = function (observable, enable) {
		if (enable && !utils.isValidatable(observable)) {
			...
			observable.errorMessage = ko.observable(null);
			...
			observable.anonymousRule = undefined;
			...
			observable._disposeValidation = function () {
				...
				if (observable.anonymousRule && observable.anonymousRule.length) {
					for (var arIndex = 0; arIndex < observable.anonymousRule.length; arIndex++) {
						var ruleName = observable.anonymousRule[arIndex];
						delete ko.validation.rules[ruleName];
					}
				}
				...
				delete observable['errorMessage'];
				delete observable['anonymousRule'];
			};
	...
	function validateSync(observable, rule, ctx) {
		...
		if (!rule.validator(observable(), ctx.params === undefined ? true : ctx.params)) { // default param is true, eg. required = true
			...
			observable.errorMessage(observable.error);
	...
	ko.validation.validateObservable = function (observable) {
		...
		observable.errorMessage = ko.observable(observable.error);
