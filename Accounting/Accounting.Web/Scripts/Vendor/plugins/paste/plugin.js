			...
			function registerEventHandlers() {
				...
				editor.on('paste', function (e) {
					...
					setTimeout(function () {
						...
						/*
						 * FIXME(kfahy): Calling getPasteBinHtml() a second time can return the
						 * pasteBinDefaultContent constant. I do not know if the second call is strictly
						 * necessary to remove excess paste bins - we will await Spocke's reply to a
						 * GitHub comment on dc8f53a, and possibly create a pull request with our
						 * changes: fallback to content if this second call returns the constant.
						 */
						var fallbackContent = content;
						content = getPasteBinHtml();
						content = Utils.trimHtml(content == pasteBinDefaultContent ? fallbackContent : content);
