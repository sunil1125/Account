...
export class VendorBillEditDetailsViewModel {
	...
	isVBReassignmentVisible: KnockoutObservable<boolean> = ko.observable(false);
	...
	// ##START: DE22284
	isNewSubBill: boolean = false;
	isNewLostBill: boolean = false;
	// ##END: DE22284

	// ##START: US21132
	hasLostBill: boolean = false;
	// ##END: US21132
	...
	totalDisputeAmountOnDisputeWon: number = $.number(0, 2);
	totalPendingCreditAmount: number = $.number(0, 2);
	bolNumberForCreditRequest: string;
	showCreditRequestAfterSave = false;
	// ##END: US25310
	...
	// ###END: US25679
	// ##START: US25684
	vendorBillReassignmentReasons: IVendorBillReassignmentReason[] = [];
	suborderRevenueList: IOrderRevenuePair[] = [];
	// ##END: US25684

	invoicedSOExceptCMSubSo: IOrderRevenuePair[] = [];

	// ###START: US26955
	actualBillCost: KnockoutObservable<number> = ko.observable(0);
		...
		self.changesDetected = (dirty: boolean) => {
			if (self.isNotAtLoadingTime === false) {
				...
				if (self.vendorBillDetailsViewModel.memo() != null && self.vendorBillDetailsViewModel.MemoexistingValue() != null
					&& self.vendorBillDetailsViewModel.memo().trim() != self.vendorBillDetailsViewModel.MemoexistingValue().trim()) {

					self.isChange(true);
				}
				// ###END: US23124

				else if (dirty || (self.vendorBillNotesViewModel.ischange())
	...
	public setUpModelAndSave() {
		...
		// ##START: US25310
		if (self.originalVendorBillContainer.VendorBill.BillStatus === refEnums.Enums.VendorBillStatus.DisputeShortPaid.ID
			&& vendorBillData.VendorBill.BillStatus === refEnums.Enums.VendorBillStatus.DisputeWon.ID) {
		...
		refVendorBillClient.VendorBillClient.prototype.SaveVendorBillDetail(vendorBillData, (message) => {
			...
			Utility.ShowToastr(refEnums.Enums.ToastrMessageType.success.ID, ApplicationMessages.Messages.SavedSuccessfullyMessage, "success", null, toastrOptions);
				...
				_app.trigger('closeActiveTab');
				_app.trigger("openVendorBill", message, vendorBillData.VendorBill.ProNumber, (callback) => {
					if (!callback) {
						return;
					}
				});
	...
	** </changeHistory>
	*/
	private showRequestCreditPopup(data: IVendorBillFinancialDetails) {
		var self = this;
	...
	private preloadCreditRequestDetails() {
		var self = this;
		...
		self.salesOrderClient.getTotalPendingCreditMemo(self.vendorBillDetailsViewModel.salesOrderId, (data: any) => {
			if (data) {
	...
	public onDisputeLost() {
		...
		// ##START: US21132
		// success callback
		var refresh = (refreshThePage = false) => {
			self.clearLocalStorage(self.vendorBillDetailsViewModel.vendorBillId() + '_' + self.vendorBillDetailsViewModel.proNumber());
	...
	public onVBReassignmentClick() {
		...
		_app.showDialog('vendorBill/VendorBillReassignmentPopup', optionControlArgs); //show popup
	}

	// ##START: US25684
	// preload the data needed on the VB reasignment popup
	private getVBReassignmentPopupData(bolNumber: string) {
		var self = this;

		if (!bolNumber)
			return;

		self.salesOrderClient.GetSubordersAndRevenuesByBOLNumber(bolNumber,
			(data: Array<IOrderRevenuePair>) => {
				if (data) {
		...
		if (typeof self.vendorBillReassignmentReasons !== 'undefined'
			&& self.vendorBillReassignmentReasons !== null
			&& self.vendorBillReassignmentReasons.length === 0) {

			self.vendorBillClient.GetVendorBillReassignmentReasons((data: IVendorBillReassignmentReason[]) => {
				if (data) {
	...
	** </createDetails>
	*/
	private getCreditMemoInvoicedSOList(shipmentId: number) {
		var self = this;
		self.salesOrderClient.GetAllInvoicedSOExceptCreditMemoSubSO(shipmentId, (data: Array<IOrderRevenuePair>) => {
			if (data) {
