...
import refPurchaseOrderColumn = require('services/client/PurchaseOrderClient');
...
** </changeHistory>
*/
export class PurchaseOrderPODDocViewModel {
	//#region Properties
	purchaseOrderPodDocDetail: KnockoutObservableArray<purchaseOrderPodDocDetails> = ko.observableArray([]);
	onAdd: () => void;
	onUpdate: () => void;
	...
	//For removing item
	removeLineItem: (lineItem: purchaseOrderPodDocDetails) => void;
	public deleteSalesOrderPODDocList: () => any;
	private selectedlineitem: purchaseOrderPodDocDetails;
	//For Tracking Change
	...
	public reportData: KnockoutObservableArray<purchaseOrderPodDocDetails> = null;
	...
	purchaseorderClientCommand: refPurchaseOrderColumn.PurchaseOrderClient = new refPurchaseOrderColumn.PurchaseOrderClient();
	//#endregion
	...
	constructor() {
		...
		self.onAdd = () => {
			...
			self.purchaseorderClientCommand.uploadAndGetUploadedResponse(uploadResquestModel, (result) => {
		...
		self.onUpdate = () => {
			...
			self.purchaseorderClientCommand.updatePodDoc(uploadFileDetails, (result) => {
				uploadLine.Id = result.Id;
				self.setPagingData(ko.observableArray(self.purchaseOrderPodDocDetail()), self.gridOptions, self.reportAction);
				self.reportContainer.listProgress(false);
				self.resetPODDOC();
		...
		self.enableUpdateButton = ko.computed(() => {
        ...
        self.reportContainer.onGridColumnClick = function (lineItem: purchaseOrderPodDocDetails) {
			if (self.isReadOnly()) {
				self.resetPODDOC();
		...
		self.fileType.subscribe((newValue) => {
		...
		self.deleteSalesOrderPODDocList = () => {
			self.purchaseorderClientCommand.deletePodDoc(self.selectedlineitem.Id, (result) => {
	...
	public initializeSalesOrderPodDocDetails(items: Array<ISalesOrderDocuments>, proNumber: string, carrierId: number, bolNumber: string, vendorBillId: number, enable?: boolean, carrierName?: string, originZip?: string) {
		var self = this;
	...
	private setGridOptions(self: PurchaseOrderPODDocViewModel): _reportViewer.ReportGridOption {
	...
	private getSalesOrderPodDocDetails(self, fileURL): purchaseOrderPodDocDetails {
...
export class purchaseOrderPodDocDetails {
	description: KnockoutObservable<string> = ko.observable();
	docOwner: KnockoutObservable<string> = ko.observable('');
	fileType: KnockoutObservable<string> = ko.observable('');
	...
	constructor(fileURL: any, strDescription: string, strDocOwner: string, strFileType: string, strCreatedDate, strUploadedBy: string, docOwnerId?: number, fileTypeId?: number, id?: number, isReadOnly?: boolean, imagePath?: string) {
		var self = this;
