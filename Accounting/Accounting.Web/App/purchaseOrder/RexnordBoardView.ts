...
class RexnordBoardViewModel {
    ...
    isMoveGTBoard: KnockoutObservable<boolean> = ko.observable(false);  //For showing "move to board" button
	...
	constructor() {
		...
		self.getReportData = function (reportActionObj: _reportViewer.ReportAction) {
			...
			if (pageno > 0) {
		       ...
		       var filterDataToSave = { UserGridSetting: saveData, GridViewId: refEnums.Enums.FilterViewName.PORexnordBoard, IsFilterApplied: self.purchaseOrderBoardsGrid.reportColumnFilter.isFilterApply, GridSearchText: self.searchText(), PageNo: self.gridOptions.pagingOptions.currentPage(), SortCol: self.sortCol(), SortOrder: self.sorttype(), FromLocalStorage: self.fromLocalStorage() };
				self.purchaseOrderClient.getPurchaseOrdersRexnordDetails(filterDataToSave,
					function (data) {
		...
		self.purchaseOrderBoardsGrid.onActionButton1Clicked = function (reportActionObj: _reportViewer.ReportAction) {
	...
	private setGridOptions(self: RexnordBoardViewModel): _reportViewer.ReportGridOption {
    ...
    // Move to GT Board
    public onMoveToGTBoard(reportActionObj: _reportViewer.ReportAction) {
        var self = this;
        var selectOpt: Array = reportActionObj.gridOptions.selectedItems();
