...
export class ReportExportControl {
	...
	getUrl: (exportType: IExportOptions, event?: Event) => any;
	onClickGenerateReport: (exportOpt: IExportOptions, event: Event) => void;
	...
	constructor(exportOpt: KnockoutObservableArray<IExportOptions>, exportGraphToPDFOption?: boolean) {
		...
		* @param {Event} : event
		*/
		self.onClickGenerateReport = function (exportOpt: IExportOptions, event: Event) {
			...
			// ##END: DE24967
				...
				if (refSystem.isFunction(self.getUrl)) {
					...
					if (resultCalback && resultCalback.done) {
						resultCalback.done((urlData, result) => {
							if (urlData == "No Data") {
						...
						if (Url == "No Data") {
							...
							$.fileDownload('DocumentViewer/DownloadReport?reportURL=' + encodedUrl)
								.done(function () {
									exportOpt.enabled(true);
								})
								.fail(function (arg) {
...
export class ReportViewerControlV3 {
	...
	/**
	* Returns visible columns from kogrid.
	*/
	public getVisibleColumns: (gridName?: string, currentTargetElement?: any, containerId?: string) => string[];
	...
	*/
	...
	constructor(header: ReportHeaderOption, grid: ReportGridOption, orientation?: string) {
		...
		*/
		self.getVisibleColumns = function (gridName?: string, currentTargetElement?: any, containerId?: string) {
			...
			var vislibleColumnList: KnockoutObservableArray<string> = ko.observableArray();
	...
	private configureActionButtons(self: ReportViewerControlV3, header: ReportHeaderOption) {
	...
	private onChange(self: ReportViewerControlV3) {
		var reportAction = new ReportAction();
	...
	public configureGrid(self: ReportViewerControlV3, options: ReportGridOption) {
